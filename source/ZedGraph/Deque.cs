...
/// <see>https://github.com/StephenCleary/Deque</see>
...
{
  /// <summary>
  /// A double-ended queue (deque), which provides O(1) indexed access, O(1) removals from the front
  /// and back, amortized O(1) insertions to the front and back, and O(N) insertions and removals
  /// anywhere else (with the operations getting slower as the index approaches the middle).
  /// </summary>
  ...
  public sealed class Deque<T> : IList<T>, System.Collections.IList
  {
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="Deque&lt;T&gt;"/> class with the elements from
    ...
    /// copied from this slice. The <see cref="T:System.Array"/> must have zero-based indexing.
    /// </param>
    ...
    /// cref="T:System.Collections.Generic.ICollection`1"/> is greater than the available space from
    ...
    /// <returns>
    /// true if <paramref name="item"/> was successfully removed from this list; otherwise, false.
    /// This method also returns false if <paramref name="item"/> is not found in this list.
    /// </returns>
    /// <exception cref="T:System.NotSupportedException">This list is read-only.</exception>
    ...
    /// <summary>
    /// Returns whether or not the type of a given item indicates it is appropriate for storing in
    /// this container.
    /// </summary>
    /// <param name="item">The item to test.</param>
    /// <returns>
    /// <c>true</c> if the item is appropriate to store in this container; otherwise, <c>false</c>.
    /// </returns>
    ...
    /// <summary>
    /// Checks the <paramref name="index"/> argument to see if it refers to a valid insertion point
    ...
    /// <summary>
    /// Checks the <paramref name="index"/> argument to see if it refers to an existing element in a
    /// source of a given length.
    /// </summary>
    /// <param name="sourceLength">
    /// The length of the source. This parameter is not checked for validity.
    /// </param>
    ...
    /// <summary>
    /// Checks the <paramref name="offset"/> and <paramref name="count"/> arguments for validity when
    /// applied to a source of a given length. Allows 0-element ranges, including a 0-element range
    ...
    /// </summary>
    private bool Full => Count == Capacity;

    /// <summary>
    /// Gets a value indicating whether the buffer is "split" (meaning the beginning of the view is
    /// at a later index in <see cref="buffer"/> than the end).
    /// </summary>
    private bool IsSplit => offset > (Capacity - Count); // Overflow-safe version of "(offset + Count) > Capacity"

    /// <summary>
    /// Gets or sets the capacity for this deque. This value must always be greater than zero, and
    /// this property cannot be set to a value less than <see cref="Count"/>.
    /// </summary>
    ...
    /// </summary>
    /// <param name="index">
    /// The zero-based view index at which the element should be inserted. This index is guaranteed
    /// to be valid.
    /// </param>
    /// <param name="item">The element to store in the list.</param>
    ...
    /// <summary>
    /// Increments <see cref="offset"/> by <paramref name="value"/> using modulo- <see
    /// cref="Capacity"/> arithmetic.
    /// </summary>
    ...
    /// <summary>
    /// Decrements <see cref="offset"/> by <paramref name="value"/> using modulo- <see
    /// cref="Capacity"/> arithmetic.
    /// </summary>
    /// <param name="value">
    /// The value by which to reduce <see cref="offset"/>. May not be negative or greater than <see cref="Capacity"/>.
    /// </param>
    ...
    /// <summary>
    /// Inserts a single element to the back of the view. <see cref="Full"/> must be false when
    ...
    /// <summary>
    /// Inserts a single element to the front of the view. <see cref="Full"/> must be false when
    /// this method is called.
    /// </summary>
    /// <param name="value">The element to insert.</param>
    ...
    /// <summary>
    /// Removes and returns the last element in the view. <see cref="Empty"/> must be false when
    /// this method is called.
    /// </summary>
    ...
    /// <summary>
    /// Removes and returns the first element in the view. <see cref="Empty"/> must be false when
    /// this method is called.
    /// </summary>
    ...
    /// The number of elements in <paramref name="collection"/>. Must be greater than zero, and the
    /// sum of <paramref name="collectionCount"/> and <see cref="Count"/> must be less than or equal
    ...
    private void DoInsertRange(int index, IEnumerable<T> collection, int collectionCount)
    {
      ...
      if (index < Count / 2)
      {
        ...
        // Move lower items down: [0, index) -> [Capacity - collectionCount, Capacity -
        // collectionCount + index) This clears out the low "index" number of items, moving them
        // "collectionCount" places down; after rotation, there will be a "collectionCount"-sized
        // hole at "index".
        int copyCount = index;
    ...
    /// <summary>
    /// Doubles the capacity if necessary to make room for one more element. When this method
    /// returns, <see cref="Full"/> is false.
    /// </summary>
    ...
    /// is not within the range [0, <see cref="Count"/>).
    /// </exception>
