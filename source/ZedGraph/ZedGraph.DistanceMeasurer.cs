...
namespace ZedGraph
{
  public class DistanceMeasurer
  {
    ...
    public DistanceMeasurer(ZedGraphControl control, Color lineColor, Color fontColor, float fontSize, CoordType coord)
    {
      m_Control              = control;
    ...
    private void MyContextMenuBuilder(ZedGraphControl control, ContextMenuStrip menuStrip, Point mousePt, ZedGraphControl.ContextMenuObjectState objState)
    {
      ...
      item1.Click += new System.EventHandler(MenuStrip_Measurer); // Add a handler that will respond when that menu item is selected
      ...
      item2.Click += new System.EventHandler(MenuStrip_ClearLast);// Add a handler that will respond when that menu item is selected
      ...
      item3.Click += new System.EventHandler(MenuStrip_ClearAll); // Add a handler that will respond when that menu item is selected
      menuStrip.Items.Add(item3);                                 // Add the menu item to the menu
    }
    protected void MenuStrip_Measurer(object sender, System.EventArgs e)
    {
      m_MeasureStateActive = !m_MeasureStateActive;
      if (m_MeasureStateActive == true)
      {
    ...
    protected void MenuStrip_ClearLast(object sender, System.EventArgs e)
    {
    ...
    protected void MenuStrip_ClearAll(object sender, System.EventArgs e)
    {
      while (m_MeasurerBeamList.Count > 0)
      {
        LineItem temp = m_MeasurerBeamList[m_MeasurerBeamList.Count - 1];
    ...
    private bool ZG_MouseDownEvent(ZedGraphControl control, MouseEventArgs e)
    {
      ...
      var point = control.GraphPane.ReverseTransformCoord(e.X, e.Y, m_Coord);
      double[] xVec = { point.X, point.X };
      double[] yVec = { point.Y, point.Y };
      m_MeasurerBeam = m_Control.GraphPane.AddCurve("", xVec, yVec, m_LineColor);
      m_MeasurerBeam.Symbol.Size = 0;
      m_MeasurerBeam.Symbol.Type = SymbolType.Circle;
      m_MeasurerBeam.Tag = "ZedGraphMeasurerLineItem";
      m_MeasurerBeam.IsY2Axis = m_Coord == CoordType.AxisXY2Scale;
    ...
    private bool ZG_MouseMoveEvent(ZedGraphControl control, MouseEventArgs e)
    {
      ...
      var point = control.GraphPane.ReverseTransformCoord(e.X, e.Y, m_Coord);
      m_MeasurerBeam.Points[1].X = point.X;
...
//                                         out m_MeasurerBeam.Points[1].X, out m_MeasurerBeam.Points[1].Y);
    ...
    private bool ZG_MouseUpEvent(ZedGraphControl control, MouseEventArgs e)
    {
      ...
      if ((m_Control.GraphPane.XAxis.Type == AxisType.Date) ||
        ...
        {
          case DateUnit.Year: ds = 1.0 / 365.0; break;
          case DateUnit.Day: ds = 1.0; break;
          case DateUnit.Hour: ds = 24.0; break;
          case DateUnit.Minute: ds = 24.0 * 60.0; break;
          case DateUnit.Second: ds = 24.0 * 60.0 * 60.0; break;
          case DateUnit.Millisecond: ds = 24.0 * 60.0 * 60.0 * 1000.0; break;
        }
      ...
      else
      {
        ...
        TextObj text = new TextObj(string.Format("{0:N3}", distance), cx, cy,
                                   m_Coord, AlignH.Center, AlignV.Bottom);
        if (!m_Control.GraphPane.YAxis.Type.Equals(m_Control.GraphPane.XAxis.Type))
        {
          ...
          if (dpx > 20.0 * dpy)
            text.Text = $"{Math.Abs(dx):N3} [ {m_Control.GraphPane.XAxis.Scale.MajorUnit}s ]";
          else if (dpy > 20.0 * dpx)
            text.Text = $"{Math.Abs(dy):N3} [ {m_Control.GraphPane.YAxis.Type} ]";
        }
        text.FontSpec.Angle = 0.0f; // Convert.ToSingle(-Math.Atan((p2.Y - p1.Y) / (p2.X - p1.X)) * 180.0 / Math.PI);
